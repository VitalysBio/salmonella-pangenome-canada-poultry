...
# group per serovar(secuence type)
ST_groups <- split(metadata$Sample, metadata$ST)
tree_grouped <- groupOTU(tree, ST_groups, group_name = "ST")

# serovar colors
colores_ST <- c("0" = "gray50", "ST11" = "#E41A1C", "ST15" = "#377EB8")
...
# plot
p <- ggtree(tree_grouped, aes(color = ST), layout = "rectangular", linewidth = 0.8) %<+% tree_data +
  geom_tiplab(aes(color = ST), size = 2.8, hjust = -0.02) +
  geom_tippoint(aes(color = ST), size = 1.5) +
  geom_text(
    aes(
      x = branch,
  ...
  labs(
    title = "Phylogenetic tree of Salmonella enterica subsp. enterica",
    color = "Serovar"
  )
...
ggsave("R_salmonella_enterica_tree_figure.pdf", p, width = 14, height = 10)
ggsave("R_salmonella_enterica_tree_figure.jpg", p, width = 14, height = 10, dpi = 300)
...
ggsave(
  "R_salmonella_enterica_tree_figure.png",
  p,
  width = 14,
  height = 10,
  dpi = 300,
